...
portlet.test.data = {
  ...
  updateStrings: {
    ...
    PortletA:
      '&~~~&PortletA&mode=view&ws=normal&parm1=Fred&parm2=Barney' + '&~~~',
    ...
    SimulateCommError:
      '&~~~&SimulateCommError&mode=view&ws=normal&parm1=Fred&parm2=Barney' +
      '&~~~',
  },
};
...
portlet.test.getIds = function() {
  ...
  let id,
    ids = [],
    pageState = portlet.test.data.initialPageState;
  for (id in pageState) {
...
(function() {
  ...
  var isInitialized = false,
    ...
    isParmInStateEqual = function(pid, state, name) {
      let newVal = state.parameters[name],
        oldVal = getParmVal(pid, name);
    ...
    setState = function(pid, state) {
      ...
      return new Promise(function(resolve, reject) {
        let simval = '';
        if (
          pid === 'SimulateCommError' &&
          state.parameters.SimulateError !== undefined
        ) {
    ...
    updatePageState = function(ustr, pid) {
      ...
      return new Promise(function(resolve, reject) {
        ...
        try {
          ...
          reject(
            new Error('Partial Action Action decode status: ' + e.message)
          );
        }
      });
    },
    ...
    getUrl = function(type, pid, parms, cache) {
      let url = 'http://www.dummyportal.com/some/context/';
      var qparms = [],
        rparms,
        qp,
        state,
      ...
      url += pid + '/' + type + '/';
      url +=
        (cache === undefined || cache === null ? 'cacheLevelPage' : cache) +
        '/?';
        ...
        for (qp in parms) {
          for (var ii = 0; ii < parms[qp].length; ii++) {
            val =
              parms[qp][ii] === null
                ? ''
                : '=' + encodeURIComponent(parms[qp][ii]);
          ...
          for (qp in rparms) {
            for (ii = 0; ii < rparms[qp].length; ii++) {
              val =
                rparms[qp][ii] === null
                  ? ''
                  : '=' + encodeURIComponent(rparms[qp][ii]);
              qparms.push(encodeURIComponent(qp) + val);
            }
          }
          qparms.push('~~~');
  ...
  portlet.impl.register = function(pid) {
    ...
    let stubs = {
       ...
       * Get allowed window states for portlet
       */
      getAllowedWS: function() {
        return getAllowedWS(pid);
      },

      /**
       * Get allowed portlet modes for portlet
       */
      getAllowedPM: function() {
        return getAllowedPM(pid);
      },
       ...
       */
      executeAction: function(parms, element, callback, onError) {
        return executeAction(pid, parms, element, callback, onError);
      },

      /**
       * Get a URL of the specified type - resource or partial action
       */
      getUrl: function(type, parms, cache) {
        return getUrl(type, pid, parms, cache);
      },
       ...
       */
      decodeUpdateString: function(ustr) {
        return updatePageState(ustr, pid);
      },
    };
    ...
    return new Promise(function(resolve, reject) {
      ...
      if (isValidId(pid)) {
        switch (pid) {
          ...
          case 'SimulateError':
            window.setTimeout(function() {
              reject(new Error('Bad error happened!'));
            }, 100);
            break;
          default:
            window.setTimeout(function() {
  ...
  portlet.test.action = {
    ...
    getState: function(url, pid) {
      let parms,
        ...
        encpid = encodeURIComponent(pid).replace(
          /[-\/\\^$*+?.()|[\]{}]/g,
          '\\$&'
        ),
        reg = '^.*&' + encpid + '(.*?)(?=&~~~).*$',
